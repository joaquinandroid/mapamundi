...
import com.example.mapamundi.modelo.PantallaOceaniaPPviewModel
import com.example.mapamundi.navigation.NavPantallas


@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun PantallaOceaniaPP(navController: NavController) {
    var situacion = LocalContext.current
    Scaffold(
        topBar = { PersonalizarTopBar26() },
        bottomBar = { PersonalizarBottomBar26() },
        content = { padding ->
            ContenidoScaffold26(padding, navController, contexto = situacion )
        }
    )
}

@Composable
fun ContenidoScaffold26(
    padding: PaddingValues,
    navController: NavController,
    europaViewModel: PantallaOceaniaPPviewModel = viewModel(),
    contexto: Context
...
fun DialogoFinal26(
    ...
    AlertDialog(
        ...
        text = {
            Text(text = "Has acertado $aciertos paises, el ${String.format("%.2f", porcentajeAciertos*100)} %")
        })
}

@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun PersonalizarTopBar26() {
    TopAppBar(
...
@Composable
fun PersonalizarBottomBar26() {
    BottomAppBar(content = {
        // Contenido de la barra inferior
        Text(text = stringResource(id = R.string.oceania))
    })
}
