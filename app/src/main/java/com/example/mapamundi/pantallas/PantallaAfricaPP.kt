...
import com.example.mapamundi.modelo.PantallaAfricaPPviewModel
import com.example.mapamundi.navigation.NavPantallas


@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun PantallaAfricaPP(navController: NavController) {
    var situacion = LocalContext.current
    Scaffold(
        topBar = { PersonalizarTopBar20() },
        bottomBar = { PersonalizarBottomBar20() },
        content = { padding ->
            ContenidoScaffold20(padding, navController, contexto = situacion )
        }
    )
}

@Composable
fun ContenidoScaffold20(
    padding: PaddingValues,
    navController: NavController,
    europaViewModel: PantallaAfricaPPviewModel = viewModel(),
    contexto: Context
...
@Composable
fun PersonalizarBottomBar20() {
    BottomAppBar(content = {
        // Contenido de la barra inferior
        Text(text = stringResource(id = R.string.africa))
    })
}
