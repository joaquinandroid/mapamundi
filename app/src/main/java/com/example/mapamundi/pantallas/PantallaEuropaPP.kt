...
import com.example.mapamundi.modelo.PantallaEuropaPPviewModel
import com.example.mapamundi.navigation.NavPantallas


@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun PantallaEuropaPP(navController: NavController) {
    var situacion = LocalContext.current
    Scaffold(
        topBar = { PersonalizarTopBar3() },
        bottomBar = { PersonalizarBottomBar3() },
        content = { padding ->
            ContenidoScaffold3(padding, navController, contexto = situacion )
        }
    )
}

@Composable
fun ContenidoScaffold3(
    padding: PaddingValues,
    navController: NavController,
    europaViewModel: PantallaEuropaPPviewModel = viewModel(),
    contexto: Context
...
@Composable
fun PersonalizarBottomBar3() {
    BottomAppBar(content = {
        // Contenido de la barra inferior
        Text(text = stringResource(id = R.string.europa))
    })
}
